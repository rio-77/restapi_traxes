...
import { getLastCheckinDistance } from '../../helper/sqliteservice'; // ganti path sesuai struktur project kamu


const db = openDatabase({ name: 'lokasi_toko.db', location: 'default' });
// tinggilayar
const { height, width } = Dimensions.get('window');
const RiwayatCio_DetailCi = ({navigation }) => {
  ...
  useEffect(() => {
    getLastCheckinDistance()
      .then((distance) => {
        if (distance !== null) {
          setJarakCheckin(parseFloat(distance));
        }
      })
      .catch((err) => console.error('Error ambil distance_in:', err));
  }, []);
      ...
      tx.executeSql(
        ...
        (_, results) => {
          if (results.rows.length > 0) {
            ...
            setFoto(data.foto_in || '');

            if (data.datetimephone_in) {
              const dateObj = new Date(data.datetimephone_in);
              const tanggalFormatted = dateObj.toLocaleDateString(); // Format: 15/4/2025
              const waktuFormatted = dateObj.toLocaleTimeString();   // Format: 14:35:12
        ...
        (error) => {
          console.log('Gagal mengambil data check-in:', error);
        }
      );
    });
  };
  ...
  return (
    <View style={styles.container}>
       <LinearGradient colors={['#204766', '#631D63']} style={styles.header}>
              ...
              <Text style={styles.headerTitle}>Berhasil Checkin</Text>
            </LinearGradient>
            ...
            {/* <TouchableOpacity style={styles.card}>
                ...
                <View style={styles.overlay}>
                  <Text style={styles.successText}> Berhasil Check-in✅ </Text>
                </View>
                </View>
             </TouchableOpacity> */}
...
{loading ? (
  ...
  >
    <Text style={styles.buttonText}>Ke Aktifitas</Text>
  </TouchableOpacity> */}
        </LinearGradient>
...
export default RiwayatCio_DetailCi;
